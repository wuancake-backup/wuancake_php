...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase {
    ...
    public static function saveBlockData($block_content, $block_tag, $template, $filepath)
    {
        ...
        if (0 == preg_match("!({$_ldl}block\s+)(name=)?(\w+|'.*'|\".*\")(\s*?)?((append|prepend|nocache)?(\s*)?(hide)?)?(\s*{$_rdl})!", $block_tag, $_match)) {
            ...
            $_name = trim($_match[3], '\'"');
            if ($_match[8] != 'hide' || isset($template->block_data[$_name])) {        // replace {$MVC.block.child}
                if (strpos($block_content, $template->smarty->left_delimiter . '$MVC.block.child' . $template->smarty->right_delimiter) !== false) {
                    if (isset($template->block_data[$_name])) {
                        $block_content = str_replace($template->smarty->left_delimiter . '$MVC.block.child' . $template->smarty->right_delimiter,
    ...
    {
        $_output = '';
        // if called by {$MVC.block.child} we must search the name of enclosing {block}
        if ($_name == null) {
            ...
            // flag that child is already compile by {$MVC.block.child} inclusion
            $compiler->template->block_data[$_name]['compiled'] = true;
        }
        if ($_name == null) {
            $compiler->trigger_template_error('{$MVC.block.child} used out of context', $compiler->lex->taglineno);
        }
        // undefined child?
