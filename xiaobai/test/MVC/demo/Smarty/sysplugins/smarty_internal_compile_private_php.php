        ...
        if ($_attr['type'] != 'tag') {
            if ($compiler->php_handling == Smarty::PHP_REMOVE) {
                ...
                if (!($compiler->smarty instanceof SmartyBC)) {
                    $compiler->trigger_template_error('$MVC->php_handling PHP_ALLOW not allowed. Use SmartyBC to enable it', null, true);
                }
                $compiler->has_code = true;
                return $_attr['code'];
            } else {
                $compiler->trigger_template_error('Illegal $MVC->php_handling value', null, true);
            }
        } else {
            $compiler->has_code = true;
            if (!($compiler->smarty instanceof SmartyBC)) {
